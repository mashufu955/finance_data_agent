...
class MetaKnowledgeService:
    ...
    async def build_meta_knowledge(self, config_file):
        ...
        async with self.meta_repository.session.begin():
            ...
            await self.meta_repository.session.execute(text('TRUNCATE TABLE column_metric'))
            await self.meta_repository.session.execute(text('TRUNCATE TABLE metric_info'))
            await self.meta_repository.session.execute(text('TRUNCATE TABLE column_info'))
            await self.meta_repository.session.execute(text('TRUNCATE TABLE table_info'))
            await self.meta_repository.session.execute(text('SET FOREIGN_KEY_CHECKS = 1'))
        logger.info('已清空旧元数据')
