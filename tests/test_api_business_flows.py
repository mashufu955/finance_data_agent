...
def test_customer_lifecycle_idempotency_scope_and_profile_tables(
    ...
    # Backdate initial status_history so the PATCH below won't hit the
    # (customer_id, changed_at, to_status) unique key within the same second.
    from datetime import datetime, timedelta
    with db_cursor() as (_, cursor):
        cursor.execute(
            "UPDATE customer_status_history SET changed_at = %s WHERE customer_id = %s",
            (datetime.now() - timedelta(seconds=2), customer["id"]),
        )
