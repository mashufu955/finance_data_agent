"""Shared test helpers -- imported by conftest and test modules directly.

This module exists to avoid the pytest conftest shadowing problem:
when two conftest.py files (tests/conftest.py and tests/test_agent/conftest.py)
both register as sys.modules['conftest'], ``from conftest import ...`` becomes
ambiguous.  Importing from ``tests._shared`` is unambiguous.
"""
...
def create_customer(api: ApiClient, *, customer_type: str = "personal") -> str:
    payload: dict[str, Any] = {
        ...
        "customer_name": f"interface-test-customer-{api.next_no('NM')}",
    ...
    if customer_type == "enterprise":
        payload.update(
            {
                "company_name": f"interface-test-enterprise-{api.next_no('CO')}",
                ...
                "business_scope": "finance data test",
                "industry": "software",
            }
        )
...
def insert_collateral(application_no: str, contract_no: str) -> str:
    ...
    with db_cursor() as (_, cursor):
        cursor.execute(
            ...
            VALUES (
                %s, %s, %s, %s, 'vehicle', 'test vehicle', 'test customer', %s,
                'CNY', 100000.00, 0.500000, 50000.00, 'test org', %s,
